...
function usend(addr, port, handlerId, data) {
    if (isNaN(handlerId) || handlerId > 0xffff) {
        logger.error('<delivery>Invalid handler ID:', handlerId);
        return;
    }
    var buf = packer.pack({ handlerId: handlerId, data: data });
...
function _onUmsg(buf) {
    var msg = packer.unpack(buf);
    var handlerId = msg.handlerId;
    var data = msg.data;
    if (!handlers[handlerId]) {
        // handler missing...
        logger.error('<delivery>Missing handler for handler ID:', handlerId);
...
function _response(bind, responseData) {
    ...
    logger.sys('<delivery>Send response to', sender, responseData);
