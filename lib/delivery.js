...
function start(conf, cb) {
    ...
    TTL = Reliable.TTL;
    reliable = new Reliable(new UdpEngine(conf.address, conf.port, conf.nic));
    _setupCleaner();
    if (!cb) {
...
function _onTimeout(buf) {
    ...
    if (type === RESP_TYPE) {
        if (responses[rid]) {
            ...
            callback(new Error('Reliable message callback timed out - handler ID:' + handlerId));
        }
        return;
    }
    // timed out message without response callback will be dropped
    logger.error('<delivery> Reliable message timed out - handler ID:', handlerId);
}
...
function _cleaner() {
    ...
    for (var rid in responses) {
        if (responses[rid].time + TTL <= now) {
            ...
            delete responses[rid];
            callback(new Error('Reliable message response timed out - handler ID: ' + handlerId));
        }   
    }
    _setupCleaner();
}
