...
function _readAnnounceValues(keys) {
    ...
    try {
        for (i = 0, len = keys.length; i < len; i++) {
            ...
            // create backup nodes
            var bklist = _createBackupNodes(parsedKey);
            // create endpoint
            tmpEndPoints.push({
...
function _createBackupNodes(parsedKey) {
    var type = parsedKey.type;
    var bkOriginNodes = parsedKey.bkOriginNodes;
    var backupNode = parsedKey.node;
    for (var i = 0, len = bkOriginNodes.length; i< len; i++) {
        var bk = bkOriginNodes[i];
        var key = bk.address + bk.port;
        if (bk.address === backupNode.address && bk.port === backupNode.port) {
            continue;
        }
        if (!backups[type]) {
            backups[type] = {};
        ...
        var add = true;
        for (var j = 0, jen = backups[type][key].length; j < jen; j++) {
            var n = backups[type][key][j];
            if (n.address === backupNode.address && n.port === backupNode.port) {
                add = false;
                break;
            }
        }
        ...
        backups[type][key].push(backupNode);
    }
    var bklist = [];
    if (backups[type] && backups[type][backupNode.address + backupNode.port]) {
        bklist = backups[type][backupNode.address + backupNode.port];
    }
    return bklist;
}
