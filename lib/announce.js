...
const TTL = Math.floor( (UPDATE_INTERVAL * 2) / 100); // this is in seconds
const SCAN_COUNT = 1000;
const MATCH = 'MATCH';
const COUNT = 'COUNT';

const values = { node: { address: null, port: 0 }, data: {}, so: 0 };
...
function _onStart(bind) {
    // initial update is faster than regular update
    setTimeout(_update, INIT_INTERVAL);
    bind.resolve();
}
