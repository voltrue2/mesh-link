...
const values = { type: 0, data: {}, bkNodes: '' };
...
function getBackupNodes(type, node) {
    ...
    if (!values.bkNodes) {
        var bkNodes = packBackupNodes(backup.get(type, node));
        values.bkNodes = bkNodes;
    }
    return unpackBackupNodes(values.bkNodes);
}
...
function _onStart(bind) {
    logger.info('<announce>Connected to Redis');
    logger.info('<announce> Use hash', useHash);
    if (started) {
        return;
    }
    started = true;
...
function handleHash() {
    slave.get(HKEY, (error, _hash) => {
        ...
        if (_hash && hash) {
            var thatSplit = _hash.split(HASH_DELIMITER);
            var thisSplit = hash.split(HASH_DELIMITER);
            var thatHash = thatSplit[0];
            var thisHash = thisSplit[0];
...
function _parseAnnounceKey(key) {
    ...
    return {
        ...
        type: list[4],
        bkNodes: unpackBackupNodes(list[5]),
