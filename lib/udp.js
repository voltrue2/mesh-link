...
class UdpEngine {
    ...
    static setNetworkInterfaceName(name) {
        conf.nic = name;
        var nic = networkInterfaces[conf.nic] || [ { address: null } ];
        conf.addr = nic[0].address || LOCALHOST;
        logger.info(
            'Target network interface is set to:', name,
            'interface:', nic[0],
            'address:', conf.addr
        );
    }
